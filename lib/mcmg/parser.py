...
class Parser:
    def __init__(self, file_name: str, data_dir: str = "Data"):
        ...
        self.score_file = (
            file_name if file_name.endswith('xml') else None
        )
    ...
    def mxl_to_xml(self, save_container: bool = False) -> None:
        ...
        target_score = self.data_dir / f"{prefix}_score.xml"
        if target_score.exists():
            self.score_file = str(target_score)
        ...
        with zipfile.ZipFile(self.file_name, 'r') as zip_ref:
            ...
            if save_container:
                if "META-INF/container.xml" in file_list:
                    container_path = self.data_dir / f"{prefix}_container.xml"
                    with zip_ref.open("META-INF/container.xml") as source, open(container_path, 'wb') as target:
            ...
            if score_path in file_list:
                target_file = self.data_dir / f"{prefix}_score.xml"
                with zip_ref.open(score_path) as source, open(target_file, 'wb') as target:
                    target.write(source.read()) #*write source.xml into prefix_source.xml
                self.score_file = str(target_file)
